...
export async function seedEnums() {
  const enums: EnumSeed[] = [
    ...
    // Notification Type
    ...NOTIFICATION_TYPE_ENUM.map((type) => ({
      enumType: ENUM_TYPE.NOTIFICATION_TYPE,
      enumValue: type,
    })),

    // Notification Read Status
    ...NOTIFICATION_READ_STATUS_ENUM.map((status) => ({
      enumType: ENUM_TYPE.NOTIFICATION_READ_STATUS,
      enumValue: status,
    })),
  ];
